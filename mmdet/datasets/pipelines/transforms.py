...
class RandomShift:
    ...
    def __call__(self, results):
        ...
        if random.random() < self.shift_ratio:
            ...
            random_shift_x = random.integers(-self.max_shift_px,
                                            self.max_shift_px)
            random_shift_y = random.integers(-self.max_shift_px,
                                            self.max_shift_px)
...
class RandomCenterCropPad:
    ...
    def _train_aug(self, results):
        ...
        while True:
            ...
            for i in range(50):
                center_x = random.integers(low=w_border, high=w - w_border)
                center_y = random.integers(low=h_border, high=h - h_border)
...
class Mosaic:
    ...
    def get_indexes(self, dataset):
        ...
        indexes = [random.integers(0, len(dataset)) for _ in range(3)]
        return indexes
...
class MixUp:
    ...
    def get_indexes(self, dataset):
        ...
        for i in range(self.max_iters):
            index = random.integers(0, len(dataset))
    ...
    def _mixup_transform(self, results):
        ...
        if padded_img.shape[0] > target_h:
            y_offset = random.integers(0, padded_img.shape[0] - target_h)
        if padded_img.shape[1] > target_w:
            x_offset = random.integers(0, padded_img.shape[1] - target_w)
...
class CopyPaste:
    ...
    def get_indexes(self, dataset):
        ...
        Returns:
            list: Indexes.
        """
        return random.integers(0, len(dataset))
...
'''Code added as part of Occlusion Copy Paste
'''
...
class OccCopyPaste:
    ...
    This implementation provides various add-ons/features as arguments:
        ...
        2. blending
        4. targeted pasting (to maximise occlusion in training)
        5. scale-aware pasting (looks at statistics of existing instances to make decision of size to paste as)
        6. contextual pasting (paste only on certain backgrounds, via COCO panoptic/stuff labels)
        7. (not exactly in this pipeline) usage of LVIS (instead of coco) segmentation masks (which is higher quality) for pasting. Not an option in this pipeline, triggered through training config via usage of mmcustom.MultiDatasetsMixDataset and mmcustom.dataset.LVISV1NewDataset.
